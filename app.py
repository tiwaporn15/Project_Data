...
with col1:
    ...
    in_nbr_floors = st.slider("จำนวนชั้น (nbr_floors)", 1, 60, 30, help="จำนวนชั้นทั้งหมดของอาคาร")
    in_year_built = st.slider("ปีที่สร้าง (year_built)", 1990, 2025, 2018, help="ปี ค.ศ. ที่สร้างเสร็จ")
    in_dist = st.number_input("ระยะทางไปรถไฟฟ้า (กม.) (dist_nearest_station)", 0.0, 20.0, 1.5, step=0.1)
    in_units = st.number_input("จำนวนยูนิต (units)", 1, 2000, 150)
    
    # Input ที่สำคัญ (เลือกเขต)
    in_district = st.selectbox("เลือกเขต (district)", sorted(KNOWN_DISTRICTS))

    st.subheader("📈 ปัจจัยเศรษฐกิจ (ปัจจุบัน)")
    
    # Input ปัจจัยเศรษฐกิจ (สมมติว่าเราป้อนค่าปัจจุบัน)
    in_unemployment = st.number_input("จำนวนผู้ว่างงาน กทม. (พันคน)", 30.0, 100.0, 60.0, step=0.1, help="อ้างอิงจากไฟล์ work2.csv")
